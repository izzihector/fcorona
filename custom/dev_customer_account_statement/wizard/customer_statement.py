...
class customer_statement(models.TransientModel):
    ...
    aging_by = fields.Selection(
        [('inv_date', 'Invoice Date'), ('due_date', 'Due Date')],
        string='Ageing By', default='due_date', required="1")
    ...
    def onchange_month(self):
        if self.month:
            ...
            if self.is_privious_year:
                month_end_date = datetime.datetime(date.year-1, a, 1) \
                    + datetime.timedelta(days=calendar.monthrange(date.year-1, a)[1] - 1)
                self.date_upto = month_end_date.date()
            else:
                month_end_date = datetime.datetime(date.year, a, 1) \
                    + datetime.timedelta(days=calendar.monthrange(date.year, a)[1] - 1)
                self.date_upto = month_end_date.date()
